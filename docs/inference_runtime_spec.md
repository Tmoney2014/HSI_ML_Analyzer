...
class ActiveBlob {
    ...
    public int[] Votes;     // 클래스별 투표수 (e.g. [PP: 100, PET: 5]) — class>=0 픽셀만
    public int TotalPixels; // 물리적 픽셀 수 (배경 제외, UNCERTAIN 포함) — CenterX 분모
    public int LastSeenLine;// 마지막으로 관측된 라인 번호 (종료 판정용)
}
List<ActiveBlob> activeBlobs = new List<ActiveBlob>();
```

#### 1.1 픽셀 분류 3-state 계약  <!-- AI가 추가함 2026-04-21: parity-fix3+4 E-M3 -->

라인 분류 결과 배열(`classificationRow`)의 각 픽셀값 계약:

| 값 | 의미 | 블롭 포함 | ClassVotes 누적 | TotalPixels 누적 |
|----|------|-----------|-----------------|-----------------|
| `-1` | **Background** — 배경으로 마스킹됨 | ❌ | ❌ | ❌ |
| `-2` | **UNCERTAIN** — 모델이 분류 불가 (confidence 미달 등) | ✅ | ❌ | ✅ |
| `>=0` | **Class N** — 확실히 분류된 클래스 | ✅ | ✅ | ✅ |

- `TotalPixels`은 물리적 blob 크기 (배경 제외, UNCERTAIN 포함) — `CenterX = MomentX / TotalPixels` 분모
- `GetBestClass()`는 `ClassVotes` 기반 다수결. 투표가 없으면 `-1` 반환 (UNCERTAIN-only blob = 미사출)
- 인식(물리적 크기/위치)과 분류(투표)가 분리됨: UNCERTAIN 픽셀은 위치에는 기여, 분류에는 기여하지 않음
